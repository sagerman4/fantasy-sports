...
FantasyRequest.prototype.isAuthenticated = function() {
    return !!this.request.session.auth.credentials.token;
};

FantasyRequest.prototype.api = function(url, type, data) {
    var deferred = Q.defer();

    if (!this.request.session.auth.credentials.token) {
    ...
    if (this.auth.isTokenExpired(this.request.session.auth.credentials.timestamp)) {
    ...
    switch (type) {
        ...
        case "PUT":
            oauth[type.toLowerCase()](url,
                this.request.session.auth.credentials.token,
                this.request.session.auth.credentials.secret,
                data,
                "application/xml",
                function(err, data) {
